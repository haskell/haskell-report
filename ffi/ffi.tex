...
% TODO:
% * "Foreign Proxy" discussion not finished yet
% * mallocForeignPtrArray and related issues
...
%\def\Version{\relax}
\def\Version{\\(Release Candidate 7)}
\begin{DRAFT}%
{
  \gdef\Version{%
    \\
    \textbf{--- DRAFT ---}\\[1ex]
    \ttfamily\scriptsize
    $\relax$Id: ffi.tex,v 1.39 2003/01/22 05:47:14 chak Exp $\relax$%
    \ignorespaces}
  }
\end{DRAFT}
...
\item[isSigned~::\ Bits a => a -> Bool]\combineitems Respectively, query the
  ...
  \code{bitSize} is undefined for types that do not have a have a fixed
  bitsize (e.g., \code{Integer}).
