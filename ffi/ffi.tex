...
Depending on the kind of import declaration there are constraints regarding
the admissible Haskell type that the variable defined in the import may have.
...
\begin{verbatim}
NOTE: The modules currently in HSLibs assert that ``arithmetic is performed
  modula 2^n''.  I think, we should not require this, as the standard doesn't
  require it for Int.
** Unless there are complains, this note will disappear in the next version. **
\end{verbatim}
...
FIXME: I am still not sure whether `withMany' isn't to esoteric to be part of
  the standard library.  It happens to be useful for marshalling in some
...
\begin{verbatim}
FIXME: Should HsFFI.h include a function like GHC's startHaskellSystem()?
\end{verbatim}
...
prototypes, and so, to access C library interfaces in Haskell.  The Haskell
system is not required to represent those types exactly as C does, but the
following guarantees are provided concerning a Haskell type \code{CT}
representing a C type \code{t}:
%
\begin{itemize}
...
\end{itemize}
%
It follows from these guarantees that all types exported by \code{CTypes} and
\code{CTypesISO} are represented as type synonyms or \code{newtype}s of basic
foreign types as defined in Section~\ref{sec:foreign-types}.
