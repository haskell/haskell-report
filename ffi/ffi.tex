...
% TODO:
% * Have an appendix with the module names under the new library naming
%   scheme?  I'd say, no.  (Rather have it in the Libraries Addendum.)

% TODO after Version 1.0:
%
% * Review suggestions by Antony Courtney <antony@apocalypse.org> re FFI
%   support for Java.
% * Modifiers for concurrency control might be needed
...
\begin{DRAFT}%
{
  \gdef\Version{%
    ...
    \textbf{--- DRAFT ---}\\[1ex]
    \ttfamily\scriptsize
    $\relax$Id: ffi.tex,v 1.18 2001/10/29 08:07:11 chak Exp $\relax$%
    \ignorespaces}
  }
\end{DRAFT}
...
\begin{verbatim}
FIXME: To be written
% We should get some people to acknowledge here as soon as the spec is
% published with a call for comments on the main list.
\end{verbatim}
...
dedicated type checker).  For example, in the case of the C calling convention
the only feasible approach would be to generate a C prototype from the Haskell
type and leave it to the C compiler to match this prototype with the prototype
for the imported function specified in the C header file.  However, the
Haskell type is lacking some information that would be required to pursue this
route.  In particular, the Haskell type does not contain any information as to
when \code{const} modifiers have to be emitted.
...
in this case, the string may be omitted in is entirety.
...
in future version of this report.
...
C header files are always included by \gterm{\#include "\gnterm{fname}"}.
There is no explicit support for \gterm{\#include <\gnterm{fname}>} style
inclusion.  The ISO C99~\cite{C99} standard guarantees that any search path
that would be used for a \gterm{\#include <\gnterm{fname}>} is also used for
\gterm{\#include "\gnterm{fname}"} and it is guaranteed that these paths are
searched after all paths that are unique to \gterm{\#include
  "\gnterm{fname}"}.  Furthermore, we require that \gnterm{fname} ends on
\code{.h} to make parsing of the specification of external entities
unambiguous.
...
mechanisms seems not to be possible here, we would have delegated the handling
of link information to the package management system of the Haskell
implementation if it had not been for multi-language intermediate forms, such
as ILX for Microsoft's .NET, which requires simple link information.  The
option to associate the name of a library archive with an external function
simplifies the generation of such intermediate code.
...
\item[complement ::\ a -> a] Calculate the bitwise complement.
  
\item[shift, rotate ::\ a -> Int -> a] Shift or rotate the bitpattern to the
  right for a positive argument and to the left for a negative argument.  The
  function \code{shift} performs sign extension on signed number types: i.e.,
  right shifts fill the top bits with 1 if the number is negative and with 0
  otherwise.
  %
  \begin{quote}
\begin{verbatim}
...
\item[bit ::\ Int -> a] Obtains a value where only the $n$th bit is set.
  
\item[setBit, clearBit, complementBit ::\ a -> Int -> a] Set, clear, or
  complement the bit at the given position.

\item[testBit ::\ a -> Int -> Bool] Check whether the $n$th bit is set.

\item[bitSize~~::\ a -> Int]
\item[isSigned~::\ a -> Bool]\combineitems Query the number of bits of values
  of type \code{a} and whether they are signed, respectively.  These functions
  never evaluate their first argument.  The function \code{bitSize} returns 0
  for types that don't have a have a fixed bitsize (eg, \code{Integer}).

\item[shiftL,~~shiftR~~::\ Bits a => a -> Int -> a]
\item[rotateL,~rotateR~::\ Bits a => a -> Int -> a]\combineitems The functions
  \code{shiftL} and \code{rotateL} are synonyms for \code{shift} and
  \code{rotate}, \code{shiftR} and \code{rotateR} negate the second argument.
...
%
\begin{codedesc}
\item[data IOErrorType] Abstract type that contains a value for each variant
  of \code{IOError}.

\item[mkIOError ::\ IOErrorType -> String -> Maybe FilePath -> Maybe Handle
  -> IOError] Construct an \code{IOError} of the given type with the second
...
\item[alreadyExistsError ::\ IOErrorType] I/O error where the operation
  failed because one of its arguments already exists.
  
\item[doesNotExistError ::\ IOErrorType] I/O error where the operation
  failed because one of its arguments does not exist.
  
\item[alreadyInUseError ::\ IOErrorType] I/O error where the operation failed
  because one of its arguments is a single-use resource, which is already
  being used.
  
\item[fullError ::\ IOErrorType] I/O error where the operation failed
  because the device is full.
  
\item[eofError ::\ IOErrorType] I/O error where the operation failed because
  the end of file has been reached.
  
\item[illegalOperation ::\ IOErrorType] I/O error where the operation is not
  possible.
  
\item[permissionError ::\ IOErrorType] I/O error where the operation failed
  because the user does not have sufficient operating system privilege to
  perform that operation.

\item[userError ::\ IOErrorType] I/O error that is programmer-defined.
...
function must be called exactly once before any Haskell function may be
invoked.  If a Haskell function is invoked earlier or \code{hs\_init()} called
multiple times, the system behaviour is undefined.
...
\code{hs\_init()}.  Note that in cases where in addition to the Haskell system
other libraries that process command line arguments during initialisation are
used, the separation of \code{hs\_init()} and \code{hs\_set\_argv()} is
essential.
