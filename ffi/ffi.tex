...
%* Rationale: Point about no lib string: remove and explain
%  why we have the [lib] prefix now
%* IOError:
%  - support for raising errors other than UserError and
%    setting the attributes like file name etc
%  - no extension of the set of "special" IO errors
%* Version 1.0: remove (put in \begin/end{DRAFT}) the non-C
%  language dependent bits and explain that all non-C calling
%  conventions are experimental and impl dependent right now
%* Clarify under which conditions two ForeignPtr are equal!
...
\begin{DRAFT}%
{
  \gdef\Version{%
    ...
    \textbf{--- DRAFT ---}\\[1ex]
    \ttfamily\scriptsize
    $\relax$Id: ffi.tex,v 1.15 2001/09/26 14:07:05 chak Exp $\relax$%
    \ignorespaces}
  }
\end{DRAFT}
...
The extension of Haskell 98 introduced in this report facilitates the use of
...
The present version of the FFI report does only fully specify the interaction
between Haskell code with code that follows the C calling convention.
However, the design of the FFI is such that it enables the modular extension
of the present definition to include other programming languages, such as C++
and Java.
