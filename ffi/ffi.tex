...
% * Should we have an appendix with the module names under the new library
%   naming scheme?  I'd say, no.  (Rather have it in the Libraries Addendum.)
...
\begin{DRAFT}%
{
  \gdef\Version{%
    ...
    \textbf{--- DRAFT ---}\\[1ex]
    \ttfamily\scriptsize
    $\relax$Id: ffi.tex,v 1.19 2001/10/30 10:19:20 chak Exp $\relax$%
    \ignorespaces}
  }
\end{DRAFT}
...
\item[shift, rotate ::\ Bits a => a -> Int -> a] Shift or rotate the bit
  ...
  number is negative and with 0 otherwise.  The operators have the following
...
\item[addForeignPtrFinalizer ::\ ForeignPtr a -> IO () -> IO ()] Add another
  finaliser to the given foreign object. No guarantees are made on the order
  in which multiple finalisers for a single object are run.
...
\item[foreignPtrToPtr ::\ ForeignPtr a -> Ptr a]
  ...
  usage occurrence of the given foreign pointer, then its finaliser(s) will be
...
\item[touchForeignPtr ::\ ForeignPtr a -> IO ()] Ensure that the foreign
  ...
  \code{ForeignPtr}s: For example, if the finaliser for one \code{ForeignPtr}
  ...
  exterior pointer must not be finalised until the interior pointer is no
  longer referenced.
...
\item[castStablePtrToPtr ::\ StablePtr a -> Ptr ()] Coerce a stable pointer to
  ...
  In particular, the address may not refer to a accessible memory location and
